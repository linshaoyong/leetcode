...
impl Solution {
    pub fn trim_mean(arr: Vec<i32>) -> f64 {
        ...
        let length = marr.len() as i32;
        let start = (f64::from(length) * 0.05) as usize;
        let end = length as usize - start;
        let sub_sum = &marr[start..end].iter().sum::<i32>();
        let sub_len = end - start;
        f64::from(*sub_sum) / f64::from(sub_len as i32)
    }
}
...
fn test_trim_mean() {
    ...
    assert_eq!(
        5.277777777777778,
        Solution::trim_mean(vec![
            9, 7, 8, 7, 7, 8, 4, 4, 6, 8, 8, 7, 6, 8, 8, 9, 2, 6, 0, 0, 1, 10, 8, 6, 3, 3, 5, 1,
            10, 9, 0, 7, 10, 0, 10, 4, 1, 10, 6, 9, 3, 6, 0, 0, 2, 7, 0, 6, 7, 2, 9, 7, 7, 3, 0, 1,
            6, 1, 10, 3
        ])
    );
    assert_eq!(
